...
CREATE DEFINER=`administrador`@`localhost` PROCEDURE `actualizarPuntoVenta` (
    IN `p_puntVen_ID` INT,
    IN `nuevo_nombre` TEXT,
    IN `nuevo_encargado` TEXT,
    IN `nueva_metaAnual` DOUBLE,
    IN `nuevo_whatsapp` TEXT,
    
    IN `nueva_ubicacion` TEXT,
    IN `nuevo_centroFormacion` TEXT
)
BEGIN
    UPDATE `punto_venta`
    SET 
