...
class TestModel extends DB
{
    ...
    public function fetchByCategory( string $category_id){
        $this->connect(); 
    $this->sql = "SELECT pi.prodImg_ruta AS rutaImagen, p.prod_nombre AS nombre, p.prod_precio * p.prod_cantidad AS precio FROM productos AS p INNER JOIN productos_imagenes AS pi ON p.prod_id = pi.prod_id WHERE p.prodCat_id = :category_id AND pi.prodImg_miniatura = 1"; ////asigna la query
